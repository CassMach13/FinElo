-- Restaura o comportamento estrito anterior: lote parcialmente ausente falha fechado.
...
begin
  ...
  where l.id = p_import_log_id and l.user_id = v_user_id
  for update;
  if not found then
  ...
  if v_detail_id_count <> coalesce(v_log.imported_count, 0) then
    raise exception 'Exclusao cancelada: rastreio exato incompleto.' using errcode = 'P0001';
  end if;

  if v_detail_id_count > 0 then
    select count(*)::integer into v_persisted_count
    from public.transactions t
    where t.user_id = v_user_id and t."ID_Transacao" = any(v_transaction_ids);

    if v_persisted_count = 0 then
      v_orphan_cleanup := true;
    elsif v_persisted_count <> v_detail_id_count then
      raise exception 'Exclusao cancelada: lote parcialmente ausente.' using errcode = 'P0001';
    else
  ...
  delete from public.import_logs l where l.id = v_log.id and l.user_id = v_user_id;
